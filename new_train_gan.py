...
from network import network, GAN, get_discriminator
...
class Callback(tf.keras.callbacks.Callback):
    def __init__(self, dataset, dataset_size, epoch, start_epoch, eval_percent,
                ...
                validation_images, optimizer, change_learning_rate, batch_size, scaling, generator):
        """
    ...
    def on_epoch_end(self, epoch, logs=None):
            ...
            while temp < count:
                ...
                if index not in indices:
                    ...
                    for x_trains, y_trains, filepaths in batch:
                        predictions = self.generator.predict(x_trains)
        ...
        for x_val, y_val, _ in validation_data:
            predictions = self.generator(x_val, training=False)
            loss_value = self.generator.loss(y_val, predictions)
        ...
        if avg_validation_loss < self.best_loss:
            ...
            try:
                self.generator.save(f'{checkpoint_path}/best_model_{self.current_epoch:04d}.keras')
            except Exception as err:
        ...
        if self.current_epoch % max(int(self.save_freq), 1) == 0:
            ...
            try:
                self.generator.save(f'{checkpoint_path}/model_{self.current_epoch:04d}.keras')
            except Exception as err:
    ...
    def on_train_end(self, logs=None):
        ...
        try:
            ...
            self.generator.save(final_model_path)
        except Exception as err:
...
def prepare_data_2(sampled_data, training, data_folder, scaling=None):
    ...
    while count < len(sampled_data):
        ...
        for index, row in sampled_data.iterrows():
            ...
            min_noise = np.min(with_noise)
            if min_noise < 0:
                with_noise -= min_noise
...
def data_augment_last(images, kwargs_data, scaling=None):
    ...
    sampled = info.sample(frac=1).drop_duplicates(subset=['dataset'], keep='first')
    if len(sampled) < x:
        rest = info[~info['patch'].isin(set(sampled['patch'].unique()))]
        n = x-len(sampled) if len(rest) >= x - len(sampled) else len(rest)
        sampled = pd.concat([sampled, rest.sample(n=n)], ignore_index=False).reset_index(drop=True)
...
def train_network(input_shape, folder, n_epochs, kwargs_data, kwargs_network, data_generator, batch_size=32, network_name='model',
    ...
    if result:
        generator, start_epoch = result
    #model.compile(optimizer=optimizer, loss=G_loss_fn)

    generator.loss = G_loss_fn
    discriminator = get_discriminator(input_shape=input_shape)
    gan = GAN(generator, discriminator)
    gan.compile(
        g_optimizer=tf.keras.optimizers.Adam(learning_rate=0.00003, beta_1=0.5),
        d_optimizer=tf.keras.optimizers.Adam(learning_rate=0.00003, beta_1=0.5),
    ...
    #Train the model
    history = gan.fit(train_dataset.map(lambda x, y, filepath: (x, y)), epochs=n_epochs-start_epoch, callbacks=[callback])   
    try:
...
def main():
    ...
    'high' : 20000,
    'location_col' : 'location',
    'abs_mean_col': 'light_mean',
    'org_abs_mean_col': 'org_light_mean',
    'median_col' : 'org_median',
    'training_path' :f'{os.path.dirname(__file__)}/{data_dir}/training',
    'eval_path' :f'{os.path.dirname(__file__)}/{data_dir}/eval',
    'data_folder' : f'{data_dir}',
    ...
    train_network(input_shape, folder, n_epochs, kwargs_data_last, kwargs_network,
                  data_generator=data_augment_last, batch_size=batch_size,
